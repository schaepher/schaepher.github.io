...
- 301 Moved Permanently  
    ...
    由于历史原因，用户代理一旦收到该状态码，后续请求可能自动将 POST 转为 GET，然后跳转到新的 URI。如果不能接受这样的转换，则应该使用 307 Temporary Redirect。  
- 302 Found  
...
- 409 Conflict  
    请求与目标资源当前的状态产生冲突。  
    常出现在对版本化的资源做 PUT 时，资源已被其他请求变更，当前请求不是基于资源最新版本变更的（ETAG 不一致）。服务端可以在响应的有效载荷里面包含帮助客户端合并两个版本的信息。  
